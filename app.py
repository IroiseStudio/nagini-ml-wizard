...
from wizard.models.registry import CLASSIFIERS, REGRESSORS
from wizard.models.train_eval import split_xy, train_pipeline
...
with gr.Blocks(title="🐍🧙 Nagini ML Wizard") as demo:
    ...
    with gr.Tab("Train"):
        task = gr.Radio(["classification","regression"], label="Task", value="classification")
        model_dd = gr.Dropdown(label="Model")
        train_btn = gr.Button("Train")
        metrics_box = gr.Textbox(label="Metrics", lines=12)
        plot_out = gr.Plot(label="Evaluation Plot")

        def on_task_change(task_sel):
            choices = list(CLASSIFIERS.keys()) if task_sel=="classification" else list(REGRESSORS.keys())
            return gr.update(choices=choices, value=choices[0])
        task.change(on_task_change, [task], [model_dd])

        def on_train(task_sel, model_name, state: WizardState):
            if not state.is_ready_for_training(): raise gr.Error("Prepare data first.")
            from wizard.preprocess import make_preprocessor
            if task_sel == "classification":
                est = CLASSIFIERS[model_name]()
            else:
                est = REGRESSORS[model_name]()
            pre, _, _ = make_preprocessor(
                state.df_clean, state.features,
                state.num_missing, state.cat_missing,
                state.scale_numeric, state.encode_categorical
            )
            X, y = split_xy(state.df_clean, state.features, state.target)
            pipe, metrics, fig = train_pipeline(
                pre, est, X, y, task_sel, test_size=state.test_size,
                random_state=state.random_state, stratify=state.stratify
            )
            state.pipeline = pipe; state.metrics = metrics; state.model_name = model_name; state.task_type = task_sel

            # format metrics text
            if task_sel == "classification":
                txt = f"Accuracy: {metrics['accuracy']:.3f}\nF1_w: {metrics['f1_w']:.3f}\n\n{metrics['report']}"
            else:
                txt = f"MAE: {metrics['mae']:.3f}\nRMSE: {metrics['rmse']:.3f}\nR²: {metrics['r2']:.3f}"
            return txt, fig, state

        train_btn.click(on_train, [task, model_dd, st], [metrics_box, plot_out, st])
